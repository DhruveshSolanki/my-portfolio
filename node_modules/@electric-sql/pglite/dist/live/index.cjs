...
`)},
                ...
                UNION ALL
                  ...
                  SELECT 
                    'DELETE' AS __op__,
                    ${g.map(({column_name:u,data_type:M,udt_name:G})=>u===o?`prev."${u}" AS "${u}"`:`NULL${M==="USER-DEFINED"?`::${G}`:""} AS "${u}"`).join(`,
`)},
                      ARRAY[]::text[] AS __changed_columns__
                  FROM prev
                ...
                UNION ALL
                  ...
                  SELECT 
                    'UPDATE' AS __op__,
                    ${g.map(({column_name:u,data_type:M,udt_name:G})=>u===o?`curr."${u}" AS "${u}"`:`CASE 
                              WHEN curr."${u}" IS DISTINCT FROM prev."${u}" 
                              THEN curr."${u}"
                              ELSE NULL${M==="USER-DEFINED"?`::${G}`:""}
                              END AS "${u}"`).join(`,
`)},
